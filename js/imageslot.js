...
(() => {
  ...
  // ── localStorage store ──────────────────────────────────────────────────
  // Images are stored as data-URLs in localStorage under LS_KEY.
  // This works on any static host (GitHub Pages, local file, etc.)
  // without needing a server or the Claude Design runtime.
  const subs = new Set();
  let slots = {};
  ...
  // Serialize writes so two near-simultaneous drops don't clobber each other.
  let saving = false;
  let saveDirty = false;
  function save() {
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Always editable — localStorage is available in any browser.
